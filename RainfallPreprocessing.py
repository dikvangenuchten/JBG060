...
def loadActualRainfall(path_to_rain_timeseries:str='rainfall/rainfall/rain_timeseries/')->pd.DataFrame:
    """Loads the files containing the actual factual rainfall data, takes as input a path to where this data is kept, should normally be
    in the rain_timeseries folder""" 
...
def predictions(location_Hirlam:str='rainfall/rainfall/',location_Harmonie:str='rainfall/rainfall/',location_rain_timeseriesdata:str='rainfall/rainfall/rain_timeseries')->pd.DataFrame:
    """loads both predictions and actual data and compiles them into one file suitable for further processing, requires the place where:
    ...
    pred_harmonie = getRainfallPredictions(loadHarmonie(location_Harmonie))
    pred_hirlam = getRainfallPredictions(loadHirlam(location_Hirlam))
    actual = cleanActualRainfall(loadActualRainfall(location_rain_timeseriesdata))
